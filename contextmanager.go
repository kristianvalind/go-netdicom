	...
	items = append(items,
		&pdu.UserInformationItem{
			Items: []pdu.SubItem{
				...
				&pdu.ImplementationClassUIDSubItem{ImplementationClassUID},
				&pdu.ImplementationVersionNameSubItem{ImplementationVersionName}}})
...
func (m *contextManager) onAssociateRequest(requestItems []pdu.SubItem) ([]pdu.SubItem, error) {
	...
	for _, requestItem := range requestItems {
		...
		case *pdu.ApplicationContextItem:
			if ri.Name != pdu.DICOMApplicationContextItemName {
				log.Printf("dicom.onAssociateRequest(%s): Found illegal applicationcontextname. Expect %v, found %v",
		...
		case *pdu.PresentationContextItem:
			...
			log.Printf("dicom.onAssociateRequest(%s): Provider(%p): addmapping %v %v %v",
	...
	log.Printf("dicom.onAssociateRequest(%s): Received associate request, #contexts:%v, maxPDU:%v, implclass:%v, version:%v",
...
func (m *contextManager) onAssociateResponse(responses []pdu.SubItem) error {
	for _, responseItem := range responses {
		...
		case *pdu.PresentationContextItem:
			...
			if ri.Result != pdu.PresentationContextAccepted {
				log.Printf("dicom.onAssociateResponse(%s): Abstract syntax %v, transfer syntax %v was rejected by the server: %s", m.label, dicomuid.UIDString(sopUID), dicomuid.UIDString(pickedTransferSyntaxUID), ri.Result.String())
			}
			if !found {
				...
				log.Printf("dicom.onAssociateResponse(%s): The server picked TransferSyntaxUID '%s' for %s, which is not in the list proposed, %v",
					m.label,
	...
	log.Printf("dicom.onAssociateResponse(%s): Received associate response, #contexts:%v, maxPDU:%v, implclass:%v, version:%v",
		m.label,
